...
class _PaymentsReceiptsScreenState extends State<PaymentsReceiptsScreen> {
  ...
  final List<Map<String, dynamic>> _paymentModes = [
    ...
    {'mode': 'Bank Transfer', 'amount': '₹3.9L', 'percent': 8, 'icon': LucideIcons.building2, 'color': const Color(0xFF8B5CF6), 'bg': const Color(0xFFF3E8FF)},
  ];
      ...
      children: [
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Payments & Receipts', style: GoogleFonts.figtree(fontSize: 22, fontWeight: FontWeight.bold, color: _dark)),
              const SizedBox(height: 4),
              Text('Browse collected payments, download receipts, and reconcile transactions.', style: GoogleFonts.figtree(fontSize: 13, color: _muted)),
            ],
          ),
        ),
        ...
        GestureDetector(
          onTap: () {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Exporting receipts...')));
          },
          child: Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text('Export Receipts', style: GoogleFonts.figtree(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.white)),
              ],
            ),
          ),
        ),
  ...
  Widget _buildSummaryCards() {
    final kpis = [
      {'label': 'Total Received', 'value': '₹48.6L', 'sub': 'May 2025', 'icon': LucideIcons.trendingUp, 'color': const Color(0xFF6366F1), 'bg': const Color(0xFFEEEDFD)},
      {'label': 'Receipts Issued', 'value': '1,284', 'sub': '+62 this week', 'icon': LucideIcons.receipt, 'color': const Color(0xFF22C55E), 'bg': const Color(0xFFF0FDF4)},
      {'label': 'Avg Payment', 'value': '₹3,787', 'sub': 'per receipt', 'icon': LucideIcons.wallet, 'color': const Color(0xFF0EA5E9), 'bg': const Color(0xFFF0F9FF)},
      {'label': 'Refunds', 'value': '₹18,400', 'sub': '6 transactions', 'icon': LucideIcons.refreshCw, 'color': const Color(0xFFEF4444), 'bg': const Color(0xFFFEF2F2)},
    ];
          ...
          child: Row(
            children: [
              Container(
                ...
                child: const Icon(LucideIcons.walletCards, size: 18, color: _primary),
              ),
              const SizedBox(width: 16),
              Text('All Modes', style: GoogleFonts.figtree(fontSize: 15, fontWeight: FontWeight.w600, color: _dark)),
              const Spacer(),
              Text('₹48.6L', style: GoogleFonts.figtree(fontSize: 16, fontWeight: FontWeight.bold, color: _dark)),
              const SizedBox(width: 8),
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 16),
              Text(m['mode'] as String, style: GoogleFonts.figtree(fontSize: 15, fontWeight: FontWeight.w600, color: _dark)),
              const Spacer(),
              Text(m['amount'] as String, style: GoogleFonts.figtree(fontSize: 16, fontWeight: FontWeight.bold, color: _dark)),
              const SizedBox(width: 8),
              const Icon(LucideIcons.chevronRight, size: 18, color: _muted),
            ],
          ),
                ...
                Stack(
                  children: [
                    ...
                    FractionallySizedBox(
                      ...
                      child: Container(height: 6, decoration: BoxDecoration(color: const Color(0xFFA5B4FC), borderRadius: BorderRadius.circular(3))),
                    ),
                  ],
                ),
                const SizedBox(height: 6),
                Text('$percent% of total', style: GoogleFonts.figtree(fontSize: 11, color: _muted)),
              ],
            ),
          ),
        ],
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Payment mode distribution is based on received amount.', style: GoogleFonts.figtree(fontSize: 12, color: _primary)),
              ...
              child: RichText(
                text: TextSpan(
                  children: [
                    ...
                    TextSpan(text: '(${_receipts.length})', style: GoogleFonts.figtree(fontSize: 16, fontWeight: FontWeight.w600, color: _muted)),
                  ],
                ),
              ),
            ),
